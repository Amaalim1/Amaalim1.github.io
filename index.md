

# Abdiaziz Maalim

Cybersecurity student at Seneca Polytechnic, completing an Honours Bachelor of
Information Technology in Cybersecurity (expected 2026). I focus on **detection
engineering, threat intelligence, and blue team operations**, with growing
interest in purple teaming and offensive security.

**Currently:** wrapping up SPR888, an AI-powered Cyber Threat Intelligence
Knowledge Base built with Python, Streamlit, ChromaDB, FastAPI, and GPT-4o-mini
— see [Projects](/projects/) for details.

**Recent experience:**
- Cyber Threat Detection Analyst (Co-op) — TD Bank, Jan–Apr 2026
- Enterprise Security Support — Dream Unlimited, Jul 2024–Mar 2025
- IT Security Support — Canadian Appliance Source

**Core stack:** Microsoft Sentinel, Splunk, Defender XDR, KQL, Python,
PowerShell — with framework grounding in MITRE ATT&CK, NIST CSF, ISO 27001,
and CIS Controls. AZ-900 certified; Security+ in progress.

Based in Toronto, open to relocating to Montreal.

---

**Find me:**
[GitHub](https://github.com/Amaalim1) ·
[LinkedIn](https://www.linkedin.com/in/YOUR-LINKEDIN) ·
[Email](mailto:you@example.com) 
